...
function inicializarEventos() {
	...
	btnDuplicar.on("click", function() {
		duplicarPantallaOrdenCompra(nroOrdenCompraOrigen.text());
	});
	...
	btnAnular.on("click", function(event) {
		if (observaciones.val().trim() == CADENA_VACIA){
			...
			mostrarDialogoAnularOrdenCompra(event);
		}		
	});
...
function verPantallaOrdenCompra(data) {
	// ****** CABECERA
	codigo.html(nroOrdenCompraOrigen.text());
	...
	if (opcion.text() == Opcion.VER) {
		...
		if (data.codigoEstadoProceso == EstadoProceso.CERRADO) {
			...
			if (data.cantidadGrAsociadas > 0) {
				...
				btnAnular.removeClass('btn-flotante-anular').addClass('btn-flotante-anular-2');
	...
	$.ajax({
		...
		success: function(resultado, textStatus, xhr) {
			...
			if (xhr.status == HttpStatus.OK) {
				...
				flgNuevo=0;
				nroOrdenCompraOrigen.text(resultado);
...
function anularOrdenCompra(){
	...
	$.ajax({
		...
		contentType: false,
		processData: false,
		url : '/appkahaxi/anularOrdenCompra/',
		data: formData,
		beforeSend: function(xhr) {
			loadding(true);
		},
...
function mostrarDialogoAnularOrdenCompra(event) {

	bootbox.confirm({
		message: "Esta operación es IRREVERSIBLE.</br>¿Está seguro que desea anular la Orden de Compra?",
		buttons: {
			confirm: {
				label: 'Sí',
				className: 'btn-success'
...
function cargarGuiaRemisionAsociada(nroGuiaRemisionOrigen) {
	var params;
...
function mostrarModalGuiasPorOrdenCompra(event) {
	...
	modalCodigoOrdenCompra.text(nroOrdenCompraOrigen.text());
...
function obtenerDetalleGuiaPorOrdenCompra(event) {
	...
	if ($.fn.dataTable.isDataTable('#tableSeleccionDocumento')) {
		...
		dataTableDetalleGuias = tableSeleccionDocumento.DataTable({
			...
			"ajax": {
				data: function(d) {
					d.codigoOrdenCompra = nroOrdenCompraOrigen.text().trim();
				},
